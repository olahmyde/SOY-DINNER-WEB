(function ($) {
	...
	function insertItemIntoCartRow(id, rowId, itemSubtitle, thumbnailPath, itemTitle, extraTitle, itemPrice) {
		...
		$('#cartItem' + id + rowId).html('<div class="order-list-img"><img src="' + thumbnailPath + '" alt=""></div><div class="order-list-details"><h4>' + itemTitle + '<br/> <small>' + itemSubtitle + extraTitle + '</small> </h4> <div style=display:none class="qty-buttons"> <input type="button" value="+" class="qtyplus" name="plus"> <input type="text" name="qty" value="1" class="qty form-control"> <input type="button" value="-" class="qtyminus" name="minus"> </div><div class="order-list-delete"><a href="javascript:;" id="deleteCartItem' + id + rowId + '"><i class="icon icon-trash"></i></a></div><div style=display:none class="order-list-price format-price">' + itemPrice.toFixed(2) + '</div></div>' + `<input type="hidden" value="${itemSubtitle}" class="qty form-control" name="order_size">`);
	...
	// Function to update item into on cart row based on: id, rowId, itemSubtitle, thumbnailPath, itemTitle, extraTitle, itemPrice
	function updateItemIntoCartRow(id, previousID, rowId, itemSubtitle, thumbnailPath, itemTitle, extraTitle, itemPrice) {

		// Remove the previous size selected.
		$("#cartItem"+id+previousID).remove();
		
		// Create the dedicated row for the cart element
		$('#itemList').append('<li id="cartItem' + id + rowId + '"></li>');

		// Insert item into its dedicated row in the cart
		$('#cartItem' + id + rowId).html('<div class="order-list-img"><img src="' + thumbnailPath + '" alt=""></div><div class="order-list-details"><h4>' + itemTitle + '<br/> <small>' + itemSubtitle + extraTitle + '</small> </h4> <div style=display:none class="qty-buttons"> <input type="button" value="+" class="qtyplus" name="plus"> <input type="text" name="qty" value="1" class="qty form-control"> <input type="button" value="-" class="qtyminus" name="minus"> </div><div style=display:none class="order-list-price format-price">' + itemPrice.toFixed(2) + '</div><div class="order-list-delete"><a href="javascript:;" id="deleteCartItem' + id + rowId + '"><i class="icon icon-trash"></i></a></div></div>' + `<input type="hidden" value="${itemSubtitle}" class="qty form-control" name="order_size">`);
	...
	function addOptionsItemToCart(id) {
		...
		thumbnailPath = '../img/food/' + id + '.jpg';

		// Capture row where the item will be inserted
		if (size == 'Small portion') {
			...
			}else if ($('#cartItem' + id + 'M').length > 0) {
				updateItemIntoCartRow(id, 'M', rowId, size, thumbnailPath, itemTitle, extraTitle, itemPrice);
				
			}else if ($('#cartItem' + id + 'L').length > 0) {
				updateItemIntoCartRow(id, 'L', rowId, size, thumbnailPath, itemTitle, extraTitle, itemPrice);
		...
		if (size == 'Medium portion') {
			...
			}else if ($('#cartItem' + id + 'S').length > 0) {
				updateItemIntoCartRow(id, 'S', rowId, size, thumbnailPath, itemTitle, extraTitle, itemPrice);
